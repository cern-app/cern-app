...
- (void) viewDidLoad
{
   ...
   // Toolbar Items
   _previousButton = [[UIBarButtonItem alloc] initWithImage : [UIImage imageNamed : @"MWPhotoBrowser.bundle/images/UIBarButtonItemArrowLeft.png"]
                                              style : UIBarButtonItemStylePlain target : self action : @selector(gotoPreviousPage)];
   _nextButton = [[UIBarButtonItem alloc] initWithImage : [UIImage imageNamed:@"MWPhotoBrowser.bundle/images/UIBarButtonItemArrowRight.png"]
...
- (void) viewDidUnload
{
   //TP: this function never gets called on iOS 6.
   _currentPageIndex = 0;
...
- (MWCaptionView *) captionViewForPhotoAtIndex : (NSUInteger) index
{
   ...
   if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
      ...
      if ([photo respondsToSelector:@selector(caption)]) {
         if ([photo caption])
            captionView = [[MWCaptionView alloc] initWithPhoto : photo];
      }
   }
...
- (void)updateNavigation {
	...
	if ([self numberOfPhotos] > 1) {
		self.title = [NSString stringWithFormat:@"%lu %@ %i", _currentPageIndex+1, NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), [self numberOfPhotos]];
	} else {
		self.title = nil;
	}
